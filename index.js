...
const jichang = [];
const requestFun = function(data, id) {
  request(data, function(error, response, body) {
    if (!error && response.statusCode == 200) {
      ...
      if (index == citys.length) {
        fs.appendFile("jichang.txt", JSON.stringify(jichang, null, 4), err => {
          if (err) {
            consola.error(err);
          }
        });
...
for (var i = 0; i < citys.length; i++) {
  options.url = baseUrl(citys[i].name);
  requestFun(options, citys[i].id);
}
